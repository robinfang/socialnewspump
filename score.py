...
class Score_Compute():
    ...
    def score(self,a,b,c):
        ...
        for i in uidtuple:
            uid = i[0]
            #权重可以调整
            self.sc_user = self.score_user(uid,1.0/3,1.0/3,1.0/3) 
            self.sc_ksum = self.score_ksum(uid)
            #权重可以调整
            sc_time = self.score_time(uid,1.0/2,1.0/2)
            sc = (a*self.sc_user+b*self.sc_ksum+c*sc_time)*100
            '''if(self.sc_user!=0)and(self.sc_ksum!=0)and(sc_time!=0):
                print uid
                print 'sc_u:'
                print self.sc_user
                print 'sc_k'
                ...
                print 'sc'
                print sc'''
            update_score = 'update weibouser set score_user = %s, score_ksum = %s, score_time = %s,score = %s where uid = %s'
            cursor.execute(update_score,(self.sc_user,self.sc_ksum,sc_time,sc,uid))
            db.commit()
        #return sc_user
            
    #用户评价指标
    def score_user(self,uid,a,b,c):
        sc_fans = self.score_fans(uid)
        sc_forwarding = self.score_forwarding(uid)
        sc_comment = self.score_comment(uid)
        self.sc_user = a*sc_fans + b*sc_forwarding + c*sc_comment
        return self.sc_user


    #根据粉丝数对用户得分
    def score_fans(self,uid):
        #找出当前表中粉丝数的最大值
    ...
    #网络结构评价指标
    def score_ksum(self,uid):
        self.ksum_compute(uid)
        #找出当前表中用户ksum的最大值
        ...
        if ksum[0]>=0.001*max_ksum[0]:
            self.sc_ksum = ksum[0]*1.0/max_ksum[0]
        else: 
            self.sc_ksum = 0
        return self.sc_ksum
        
    #ksum 计算和更新
    def ksum_compute(self,uid):
        new_uid = []
        ksum_each = 0
        ksum = 0
    ...
    #时间评价指标
    def score_time(self,uid,a,b):
        sc_latest_time = self.score_latest_time(uid)
        sc_over_time = self.score_over_time(uid)
        sc_time = a*sc_latest_time + b*sc_over_time
        return sc_time

    #最新微博发布时间评分
    def score_latest_time(self,uid):
...
if __name__ == '__main__':
    ...
    test =Score_Compute()
    test.score(1.0/3,1.0/3,1.0/3) #权重可以调整